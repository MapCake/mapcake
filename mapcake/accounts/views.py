    ...
    **Context**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: redirect_to = reverse('accounts:userena_signup_complete',
    ...
    try:
        if (not UserenaSignup.objects.check_expired_activation(activation_key)
            ...
            if user:
                ...
                else: redirect_to = reverse('accounts:userena_profile_detail',
                                            kwargs={'username': user.username})
                return redirect(redirect_to)
            else:
                if not extra_context: extra_context = dict()
...
def activate_retry(request, activation_key,
    ...
    if not userena_settings.USERENA_ACTIVATION_RETRY:
        return redirect(reverse('accounts:userena_activate', args=(activation_key,)))
    try:
...
def email_confirm(request, confirmation_key,
    ...
    if user:
        ...
        if success_url: redirect_to = success_url
        else: redirect_to = reverse('accounts:userena_email_confirm_complete',
                                    kwargs={'username': user.username})
        return redirect(redirect_to)
    else:
        if not extra_context: extra_context = dict()
...
def signout(request, next_page='/',
            template_name='home/index.html', *args, **kwargs):
    """
    ...
    **Todo**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: redirect_to = reverse('accounts:userena_email_change_complete',
                                        kwargs={'username': user.username})
            return redirect(redirect_to)
    ...
    **Context**
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if success_url: redirect_to = success_url
            else: redirect_to = reverse('accounts:userena_password_change_complete',
                                        kwargs={'username': user.username})
            return redirect(redirect_to)
        ...
        if form.is_valid():
            ...
            if success_url:
                ...
                userena_signals.profile_change.send(sender=None,
                                                    user=user)
                redirect_to = success_url
            else: redirect_to = reverse('accounts:userena_profile_detail', kwargs={'username': username})
            return redirect(redirect_to)
